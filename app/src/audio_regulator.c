...
sc_audio_regulator_push(struct sc_audio_regulator *ar, const AVFrame *frame) {
    ...
    // The buffering level is measured via can_read, which is read AFTER all
    // buffer operations (writing resampled samples, skipping on overflow).
    // Therefore can_read already reflects the regulator's own actions
    // (compensation samples written, overflow samples dropped), and no
    // instant adjustment is needed.
    //
    // Previously, an instant adjustment (avg += written - input_samples +
    // underflow - dropped) was applied BEFORE the EMA push, but since
    // can_read already included these effects, the adjustment was double-
    // counted and then immediately diluted by the EMA (retaining only ~0.78%
    // of the adjustment). This systematic underestimation of compensation
    // caused the average buffering to drift over long recordings (>1 hour),
    // eventually triggering overflow protection and audible glitches.
